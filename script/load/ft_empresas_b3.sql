...
SELECT 
  ...
  ,dim_cnae.id dim_cnae_id
  ,dim_ramo_atividade.id dim_ramo_atividade_id
  ,dim_setor.id dim_setor_id
FROM  `vivid-science-452722-u2.bronze.bz_df_empresas` df 
INNER JOIN `vivid-science-452722-u2.fato.dim_cnae` dim_cnae
  ON df.de_cnae_principal = dim_cnae.de_cnae_principal 
INNER JOIN `vivid-science-452722-u2.fato.dim_ramo_atividade` dim_ramo_atividade
  ON df.de_ramo_atividade = dim_ramo_atividade.de_ramo_atividade 
INNER JOIN `vivid-science-452722-u2.fato.dim_setor` dim_setor
  ON df.de_setor = dim_setor.de_setor 
